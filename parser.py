...
ИНН_КОМПАНИЯ = {
    "7721632827": "АО Концерн Росэнергоатом",
    "7705035012": "ПАО МосЭнерго",
    "2460066195": "ПАО РусГидро",
    "8602067092": "АО Юнипро",
    "6315376946": "ПАО Т Плюс",
    "6671156423": "ПАО Энел Россия",
    "3800000220": "ПАО Иркусткэнерго",
    "7203162698": "ПАО Фортум",
    "6829012680": "ПАО Квадра",
    "7706697347": "АО ЕвроСибЭнерго",
    "2460237933": "АО Красноярская теплотранспортная компания",
    "8901025421": "АО ЯМАЛКОММУНЭНЕРГО",
    "0570006131": "ООО Дагестанэнерго",
    "6670129804": "АО Свердловская энергогазовая компания",
    "7703806647": "АО ГТ Энерго",
    "7704863782": "ООО ВО Технопромэкспорт",
    "7706593549": "ООО Ситиэнерго",
    "2204052762": "АО БИЙСКЭНЕРГО",
    "4101023932": "АО ГЕОТЕРМ",
    "1901064509": "ООО Абаканэнергосбыт",
}
...
for инн in ИНН_КОМПАНИЯ.keys():
    ...
    for год in ("2015", "2016", "2017"):
        данные[инн][год] = pd.Series({
            показатель: float(баланс[инн][год].get(str(код), "NaN")) for показатель, код in ПОКАЗАТЕЛЬ_КОД.items()
        })
        чистая_прибыль[инн][год] = [float(баланс[инн][год].get("2400", "NaN"))]


with pd.ExcelWriter(НАЗВАНИЕ_EXCEL_ФАЙЛА, engine="openpyxl") as writer:
