...
АДРЕСС_API = "https://damia.ru/api-rs/balance"
...
def сделать_запрос(адресс, параметры={}):
    r = requests.get(адресс, params=параметры)
    r.raise_for_status()
    return r.json()
...
for инн in ИНН_КОМПАНИЯ.keys():
    параметры["inn"] = инн
    баланс = сделать_запрос(АДРЕСС_API, параметры)[str(инн)]
